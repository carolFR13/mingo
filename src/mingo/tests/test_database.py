...
def check_database_content(db: Database) -> None:
    ...
    with db.engine.connect() as conn:
        ...
        for idx, result in enumerate(planes_data):
            ...
            assert isinstance(result[4], float | None)
            assert isinstance(result[5], str | None)
            assert isinstance(result[6], float | None)

        # Check data in event table
        _data = conn.execute(select(db.event)
                             .order_by(db.event.c.e_0)
                             .order_by(db.event.c.id)).fetchall()
...
def test_Database_load() -> None:
    ...
    _db = Database(DEFAULT_INPUT, ask_to_create=False)
    db = Database(DEFAULT_INPUT)
...
def test_Database_fill(make_sources: list[Path], input_format: str) -> None:
    ...
    try:
        ...
        loaded_db = Database(DEFAULT_INPUT)
...
def test_plane_id_uniqueness() -> None:
    ...
    try:
        planes = [
            db.PlaneInput(0, 0, 0, None, None, None),
            db.PlaneInput(1, 0, 0, None, None, None),
            db.PlaneInput(2, 0, 0, None, None, None),
            db.PlaneInput(3, 0, 0, None, None, None),
            db.PlaneInput(4, 0, 0, None, None, None),
            db.PlaneInput(5, 0, 0, None, None, None)
        ]

        expected_plane_data = [
            (1, 0, 0, 0, None, None, None),
            (3, 1, 0, 0, None, None, None),
            (4, 2, 0, 0, None, None, None),
            (6, 3, 0, 0, None, None, None),
            (7, 4, 0, 0, None, None, None)
        ]
...
def test_config_id_uniqueness() -> None:
    ...
    try:
        planes = [db.PlaneInput(0, 0, 0, None, None, None),
                  db.PlaneInput(1, 0, 0, None, None, None)]
        db.insert_plane(planes)

        configs = [
            db.ConfigInput(1, 1, 1, 1, 0, 0, 0, 0),
            db.ConfigInput(1, 1, 1, 2, 0, 0, 0, 0),
            db.ConfigInput(1, 1, 2, 1, 0, 0, 0, 0),
            db.ConfigInput(1, 2, 1, 1, 0, 0, 0, 0),
            db.ConfigInput(2, 1, 1, 1, 0, 0, 0, 0)
        ]
